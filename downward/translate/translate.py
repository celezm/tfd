...
def strips_to_sas_dictionary(groups, num_axioms, num_axiom_map, num_fluents):
    ...
    for axiom in num_axioms:
        if axiom.effect in num_axiom_map:
            ...
            dictionary.setdefault(axiom.effect, []).append(
                (num_ax_count + len(groups), -2)
            )
            num_ax_count += 1
    ...
    if temporal:
        condition = [
            translate_strips_conditions_aux(
                conds, dictionary, ranges, comp_axioms, true_atoms, false_atoms
            )
        ...
        return translate_strips_conditions_aux(
            conditions, dictionary, ranges, comp_axioms, true_atoms, false_atoms
        )


def translate_strips_conditions_aux(
    conditions, dictionary, ranges, comparison_axioms, true_atoms, false_atoms
):
    ...
    for fact in conditions:
        if isinstance(fact, pddl.FunctionComparison) or isinstance(
            fact, pddl.NegatedFunctionComparison
        ):
            if fact not in dictionary:
                ...
                if key in comp_axiom_dict:
                    ...
                    axiom = sas_tasks.SASCompareAxiom(
                        fact.comparator, parts, len(ranges)
                    )
    ...
    for fact in negated_facts:
        ...
        if not done and new_condition:
            ...
            # smalles one.
            candidates = sorted(
                list(new_condition.items()), key=lambda x: len(x[1])
            )
    ...
    def multiply_out(condition):  # destroys the input
        sorted_conds = sorted(
            list(condition.items()), key=lambda x: len(x[1])
        )
        flat_conds = [{}]
        ...
        for cond in condition_list:
            ...
            for time in range(3):
                for (var, val) in cond[time]:
                    if (var, val) not in condition[time] and global_cond[time].get(
                        var
                    ) != val:
                        triggers = False
                        break
...
def translate_add_effects(
    add_effects, dictionary, ranges, comp_axioms, temporal, true_atoms, false_atoms
):
    assert temporal
    effect = {}
    ...
    for conditions, fact in add_effects:
        eff_condition_dict_list = translate_strips_conditions(
            conditions,
            dictionary,
            ranges,
            comp_axioms,
        ...
        if eff_condition_dict_list is None:  # Impossible condition for this effect.
            continue
        eff_condition_temporal_dicts = cartesian_product_temporal_conditions(
            eff_condition_dict_list
        )
        # now eff_condition_temporal_dicts is a list of temporal conditions

        if temporal:
            eff_conditions = [
                [list(eff_dict.items()) for eff_dict in eff_cond]
                for eff_cond in eff_condition_temporal_dicts
            ]
        else:
            eff_conditions = [
                list(eff_dict.items()) for eff_dict in eff_condition_temporal_dicts
            ]
                ...
                if other_val != val:
                    for other_cond in hitherto_effect[other_val]:
                        for eff_cond in eff_conditions:
                            # redictify
                            eff_cond_as_dict = [
                                dict(eff_pairs) for eff_pairs in eff_cond
                            ]
                            if not mutex_conditions(
                                eff_cond_as_dict, other_cond, temporal
                            ):
            ...
            for eff_condition in eff_conditions:
                ...
                for other_val, other_eff_conditions in list(hitherto_effects.items()):
                    if other_val != none_of_those:
                        if implies(
                            eff_condition, other_eff_conditions, condition, temporal
                        ):
                            found_matching_add_effect = True
                            break
                        for cond in other_eff_conditions:
                            if not mutex_conditions(eff_cond_as_dict, cond, temporal):
                ...
                already_undefined = (
                    condition[cond_time].get(var) == none_of_those
                    or eff_cond_as_dict[cond_time].get(var) == none_of_those
                )
                if found_matching_add_effect or already_undefined:
                    ...
                    if (
                        condition[cond_time].get(var) != val
                        and eff_cond_as_dict[cond_time].get(var) != val
                    ):
                        ...
                        if var in condition[cond_time] or (
                            cond_time == 2
                            and var in condition[1]
                            and condition[1][var] != val
                        ):
                            continue
...
def translate_assignment_effects(
    assign_effects, dictionary, ranges, comp_axioms, temporal, true_atoms, false_atoms
):
    assert temporal
    effect = {}
...
def translate_strips_operator(operator, dictionary, ranges, comp_axioms):
    ...
    condition = translate_strips_conditions(
        operator.condition, dictionary, ranges, comp_axioms
    )
    if condition is None:
        return None

    effect, possible_add_conflict = translate_add_effects(
        operator.add_effects, dictionary, ranges, comp_axioms, False
    )
    translate_del_effects(
        operator.del_effects,
        dictionary,
        ranges,
        effect,
        condition,
    ...
    assign_effect, possible_assign_conflict = translate_assignment_effects(
        operator.assign_effects, dictionary, ranges, comp_axioms, False
    )
...
def translate_temporal_strips_operator_aux(
    operator, dictionary, ranges, comp_axioms, condition, true_atoms, false_atoms
):
    # NOTE: This function does not really deal with the intricacies of properly
    # encoding delete effects for grouped propositions in the presence of
    # conditional effects. It should work ok but will bail out in more
    ...
    possible_add_conflict = False
    for time in range(2):
        eff, poss_conflict = translate_add_effects(
            operator.add_effects[time],
            dictionary,
            ranges,
            comp_axioms,
            True,
    ...
    for time in range(2):
        for var in assign_effect[time]:
            for ((op, valvar), eff_condition_lists) in assign_effect[time][var].items():
                for eff_condition in eff_condition_lists:
                    sas_effect = sas_tasks.SASAssignmentEffect(
    ...
    return sas_tasks.SASTemporalOperator(
        operator.name, duration, prevail, pre_post, assign_effects
    )


def translate_temporal_strips_operator(
    operator, dictionary, ranges, comp_axioms, true_atoms, false_atoms
):
    condition = translate_strips_conditions(
        operator.conditions,
        dictionary,
    ...
    for temp_cond in temp_conds:
        op = translate_temporal_strips_operator_aux(
            operator,
            dictionary,
            ranges,
            comp_axioms,
...
def translate_strips_axiom(axiom, dictionary, ranges, comp_axioms):
    ...
    conditions = translate_strips_conditions(
        axiom.condition, dictionary, ranges, comp_axioms
    )
...
def translate_temporal_strips_operators(
    actions, strips_to_sas, ranges, comp_axioms, true_atoms, false_atoms
):
    result = []
    actions.sort(key=lambda x: x.name)
    for action in actions:
        sas_ops = translate_temporal_strips_operator(
            action, strips_to_sas, ranges, comp_axioms, true_atoms, false_atoms
        )
        if sas_ops:
...
def translate_task(
    ...
    (
        ...
        true_atoms,
        false_atoms,
    ) = axiom_rules.handle_axioms(actions, durative_actions, axioms, goals)

    init = init + axiom_init

    # filter trivial true_atoms from goal
    goals = [
        g for g in goals if g not in true_atoms
    ...
    goal_dict_list = translate_strips_conditions(
        goals, strips_to_sas, ranges, comp_axioms
    )
    ...
    temp_operators = translate_temporal_strips_operators(
        durative_actions, strips_to_sas, ranges, comp_axioms, true_atoms, false_atoms
    )

    axioms = translate_strips_axioms(axioms, strips_to_sas, ranges, comp_axioms)
    sas_num_axioms = [
        translate_numeric_axiom(axiom, strips_to_sas)
        for axiom in num_axioms
...
def pddl_to_sas(task):
    ...
    ranges, strips_to_sas = strips_to_sas_dictionary(
        groups, num_axioms, num_axiom_map, num_fluents
    )
